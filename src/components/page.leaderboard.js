       ...
       let dir = this.state.language == "fa" ? "rtl text-right" : "ltr text-left"
        let html = ""
				...
				if(data.users.length>0)
				{
                    ...
                    html +=   `<div class="${containerClass}"><img class="leader-board-logo" src="${AppConfig.baseUrl}images/profileImages/${item.logo}.png" /></div>
                                   <div style="max-width:30%;overflow-x:hidden;" class="text-right">${item.username}</div>
                                  <div class="text-right">${item.score}</div>
                                  </div>`
					})
				}
			else
				html=`<div>${AppConfig.dictionary.noRecord[language]}</div>`
